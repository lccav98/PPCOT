        ...
        <>
          ...
          <div className="bg-card-bg rounded-lg p-4 border border-military-green">
            <div className="flex justify-between items-center mb-3">
              ...
              <button onClick={addCriterio} className="btn-secondary text-xs flex items-center gap-1"><Plus size={12}/> Adicionar</button>
            </div>
            <div className="space-y-2">
              {f.criterios.map(c => (
                <div key={c.id} className="flex gap-2 items-center">
                  ...
                  <button onClick={() => upd({ criterios: f.criterios.filter(x => x.id !== c.id) })} className="text-red-600"><Trash2 size={14}/></button>
                </div>
              ))}
            </div>
          </div>
                      ...
                      return (
                        <td key={la.id} className="text-center py-2 px-3">
                          <div className="flex flex-col items-center gap-1">
                            <select className="bg-dark-bg border border-green-800 rounded px-1 py-0.5 text-green-200 w-14 text-center"
          ...
          <div className="bg-card-bg rounded-lg p-4 border border-military-green">
            ...
            <div className="space-y-2">
              {ranking.map((la, i) => {
                ...
                return (
                  <div key={la.id} className={`flex items-center gap-3 p-3 rounded border ${isRec ? 'border-military-gold bg-military-green/30' : 'border-green-900'}`}>
                    <span className={`font-bold text-sm w-6 ${i === 0 ? 'text-military-gold' : 'text-green-600'}`}>{i + 1}º</span>
                    <span className="text-green-200 flex-1 text-sm">L Aç {la.numero} — {la.sumario.substring(0, 50) || la.oQue.substring(0, 50) || 'Sem sumário'}</span>
                    <div className="w-32 bg-green-900 rounded-full h-2">
                      <div className="bg-military-gold h-2 rounded-full" style={{ width: `${pct}%` }} />
                    </div>
                    <span className="text-military-gold text-sm font-bold w-10 text-right">{total}</span>
                    <button onClick={() => setRecomendada(la.id)}
                      className={`flex items-center gap-1 text-xs px-2 py-1 rounded border ${isRec ? 'bg-military-gold text-military-green border-military-gold font-bold' : 'border-green-700 text-green-500 hover:border-military-gold'}`}>
